...
def fetch_posts(channel_id):
    ...
    # User-Agent de um dispositivo Android para forçar a versão mobile
    headers = {
    ...
    posts = []
    try:
        # Acesso direto à estrutura mobile
        contents = data.get("contents", {}).get("singleColumnBrowseResultsRenderer", {}).get("tabs", [])
        for tab in contents:
